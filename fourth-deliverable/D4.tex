...
\begin{itemize}
	...
	\item "Express": framework Back-End per applicazioni web in javascript. Il logo è formato dalle due lettere "e" e "x" in nero.
	\item "React": framework Front-End lo sviluppo di applicazioni web. L'icona ricorda un atomo.
\end{itemize}
...
Il linguaggio scelto per la Back-End e Front-End è \textit{Typescript}: un linguaggio tipizzato che viene tradotto da un compilatore ("transpiler") in Javascript e dunque può essere eseguito in un web browser. Qualsiasi script JavaScript è anche codice TypeScript valido, questo permette all'applicazione di utilizzare l'ampia collezione di pacchetti JavaScript.

\subsection{Frameworks e Dipendenze}

Ogni microservizio utilizza:
\begin{itemize}
	\item \textit{ts-node} v10.9.2: ambiente runtime Typescript open source e multipiattaforma per la creazione del server di Back-End.
	...
	\item \textit{React} v18.3.0: libreria UI per javascript, utilizzata per la Front-End.
	...
	\item \textit{concurrently} v8.2.2: esegue più comandi contemporaneamente, utilizzato per eseguire  e Back-End insieme.
	\item \textit{serve} v14.2.3: hosta un sito statico, utilizzato per hostare la Front-End in produzione.
...
\begin{itemize}
	\item "backend": cartella contenente tutto il codice di Back-End in TypeScript.
	...
	\item "dist": cartella contenente il codice Back-End per la produzione, generato dal compilatore di TypeScript.
	...
	\item "package.json": file che definisce le dipendenze e le impostazioni di vari moduli.
	...
	\item "public": contiene file di accesso pubblico per la Front-End, come favicon.ico, robots.txt, index.html.
	...
	\item "tailwind.config.js": file di configurazione per il funzionamento di tailwind CSS
...
\begin{itemize}
	...
	\item enums/ : directory contenente gli enum necessari.
...
\begin{itemize}
	\item "startfront": esegue solo la Front-End, si aggiorna automaticamente ad ogni modifica del codice
	\item "buildfront": genera la Front-End statica per la production
	\item "startback": esegue soltanto il server di Back-End, si aggiorna automaticamente ad ogni modifica del codice
	\item "start": esegue sia la Front-End sia la Back-End in developement mode
	\item "production": esegue sia la Front-End sia la Back-End in production mode
	\item "lint": esegue un'analisi statica del codice
	\item "test": esegue gli script di testing del codice.
\end{itemize}

\subsection{Elementi Front-End}
...
\subsection{Struttura della Back-End}
La struttura della Back-End del microservizio autenticazione è riportata
nella figura sotto.
\begin{figure}[H]
...
\subsection{Struttura del Front-End}
La struttura del Front-End del microservizio autenticazione è riportata
nella figura sotto.
\begin{figure}[H]
...
Questa sezione comprende lo sviluppo e la documentazione della Back-End del
Microservizio "Task".
...
\subsection{Struttura del Front-End}
La struttura del Front-End del microservizio task è riportata
nella figura sotto.
\begin{figure}[H]
...
Questa sezione comprende lo sviluppo e la documentazione del Back-End del Microservizio 2Gestione Dipendenti"

\subsection*{Struttura del Back-End}
La struttura della Back-End del microservizio "Gestione Dipendenti" è riportata
nella figura sotto.
\begin{figure}[H]
	\centering\includegraphics[width=0.5\textwidth]{images/microservizio-autenticazione/backend-structure.png}
	\caption{Output del comando "tree" all'interno di un microservizio.}
...
\subsection{Struttura del Front-End}
La struttura della Front-End del microservizio Home è riportata
nella figura sotto.
\begin{figure}[H]
...
\begin{figure}[H]
	\centering\includegraphics[width=1\textwidth]{images/microservizio-home/userflow-home.png}
...
\begin{figure}[H]
	\centering\includegraphics[width=1\textwidth]{images/microservizio-home/frontend/homepage-userflow.png}
